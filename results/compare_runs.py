...
def summarize(run_dir: Path, history: dict) -> dict:
    ...
    last_with_test = next(r for r in reversed(rows) if "mean_test_nse" in r)
    return {
        "run": run_dir.name,
        "model": history["model"],
        "n_epochs": history["n_epochs"],
        "final_train_nse": rows[-1]["mean_train_nse"],
        "final_test_nse": last_with_test["mean_test_nse"],
        "train_test_gap": rows[-1]["mean_train_nse"] - last_with_test["mean_test_nse"],
...
def main() -> None:
    ...
    header = (
        f"{'run':<{name_w}}  {'model':<14}  {'epochs':>6}  "
    ...
    for s in summaries:
        print(
            f"{s['run']:<{name_w}}  {s['model']:<14}  {s['n_epochs']:>6}  "
    ...
    for i, (run_dir, history) in enumerate(histories.items()):
        ...
        ax.plot(
            [r["epoch"] for r in rows], [r["mean_train_nse"] for r in rows],
            color=color, linestyle="-", label=f"{run_dir.name} (train)",
        )
        test_rows = [r for r in rows if "mean_test_nse" in r]
        ax.plot(
            [r["epoch"] for r in test_rows], [r["mean_test_nse"] for r in test_rows],
